...
>### TLDR version
> To use lazy initialization, which will resolve the source of the state only once,
in order to avoid unnecessary computation during re-render.
...
Each time we click the button the `complexFunction` will always being called(_verify by uncommenting console.log or alert_).
...
For a such big word like `lazy initialization` it actually is very simple. Simply pass a function instead of a value.
```jsx
const [,setState] = React.useState(() => complexFunction())
// OR
const [,setState] = React.useState(complexFunction)
...
it could make our application "slower" in contrast to our goal to make it faster.
...
`lazy initialization` has a small overhead, so use it wisely.
