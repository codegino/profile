...
## If your unit test is slow, you are probably doing something wrong.

<HalfScreen />

# Why are unit tests running slow in my pipeline?

<HalfScreen />

# Too many unit tests... 🤯
<h2 style={{textAlign: 'center'}}>(Too many unnecessary unit tests)</h2>
...
test('Product Form should update store status', () => {
	...
	expect(productForm.getStatus('4712052989979')).toBe(true)
});
```
...
## As time passes, developers come and go; other developers add features, fix bugs, and add new tests.
## However, some developers won't dare touch the old unit tests that are already working.
...
<h1 style={{color: 'red'}}>Warning!️!️!️</h1>
...
# It is easier said than done, isn't it? 🤦🏻‍♂️
...
<h3 style={{textAlign: 'center'}}>Guess how many tests it needs to get a high coverage</h3>
...
# How were we able to achieve this with fewer test cases?
...
# [KISS (keep it simple, stup🤬d)](https://en.wikipedia.org/wiki/KISS_principle)
