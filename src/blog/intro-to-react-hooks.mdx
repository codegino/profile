...
> #### From the official [documentation](https://reactjs.org/docs/hooks-intro.html)
...
And `functional components` were mostly used as a dumb component, where
...
Let's try to mimic the all the implementation of the class component to our function component.
...
You can modifly the component all day long, but it won't react to state changes.
<br />

What if we can have the simplicity of the `functional component` and still have
the ability to react state changes? React hooks to the rescue.

Here most common examples of react hooks
- useState
- useEffect
- useRef
- useCallback
...
As per the title of this blog, I will only discuss `useState`, which is the solution related to our previous problem.
...
As per the examples above, here's what you can pass in `useState` as an argument
- nothing
- primitive value
- object or array
...
Let's replicate the behavior of the class component in a function component using `useState` hook.
...
For me, I always use the `functional component` approach, but if I'm working on a very old codebase where there's no way to migrate to the newer version,
then I will definitely use a `class component`.
Also, it is written in the React documentation that `There are no plans to remove classes from React`.
There are some use cases like usage of Inheritance, creating Error Boundary, explicit lifecycle methods, etc.
that cannot be directly done using a `functional component`, although there are many workarounds to apply the expected behavior.

<br />
<br />

## Conclusion
Using `useState` will simplify how we can create a component.
